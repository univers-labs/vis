...
class InteractionHandler {
   ...
   * @private
   */
 _checkShowPopup(pointer, clicked = false) {
    ...
    if (this.popupObj !== undefined) {
      ...
      if (this.popupObj.options.inactive && !clicked) {
        ...
        if (popupType === 'edge') {
          let fromObj = this.popupObj.from;
          let toObj = this.popupObj.to;

          if (this.ancillaryPopups.from === undefined) {
            this.ancillaryPopups.from = new Popup(this.canvas.frame);
            this.ancillaryPopups.to = new Popup(this.canvas.frame);
          }

          let fromPopup = this.ancillaryPopups.from;
          let toPopup = this.ancillaryPopups.to;
  ...
  _positionPopups(pointer) {
    let x = this.canvas._XconvertDOMtoCanvas(pointer.x);
    let y = this.canvas._YconvertDOMtoCanvas(pointer.y);
    let px, py;

    let popupType = this.popup.popupTargetType;
    ...
    if(popupType === 'node') {
      ...
      let afx = this.canvas._XconvertCanvasToDOM(fromObj.x);
      let afy = this.canvas._YconvertCanvasToDOM(fromObj.y);

      let atx = this.canvas._XconvertCanvasToDOM(toObj.x);
      let aty = this.canvas._YconvertCanvasToDOM(toObj.y);

      let distance = Math.sqrt(Math.pow(atx - afx, 2) + Math.pow(aty - afy, 2));
      ...
      let width = this.popup.frame.clientWidth;
      if(distance < width) {
        ...
        let angle = Math.atan2((fromObj.y - toObj.y), (fromObj.x - toObj.x));

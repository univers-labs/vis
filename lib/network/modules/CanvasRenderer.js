...
class CanvasRenderer {
  ...
  _redraw(hidden = false) {
    if (this.allowRedraw === true) {
      ...
      let nodesMap = this.body.nodes;
      let nodeIndices = this.body.nodeIndices;
      let nodes = nodeIndices.map((n)=> nodesMap[n] );
      let inactiveNodes = nodes.filter((n)=> n.options.inactive );
      let activeNodes = nodes.filter((n)=> !n.options.inactive );

      let edgesMap = this.body.edges;
      let edgeIndices = this.body.edgeIndices;
      let edges = edgeIndices.map((e)=> edgesMap[e] );
      let inactiveEdges = edges.filter((e)=> e.options.inactive );
      let activeEdges = edges.filter((e)=> !e.options.inactive );

      if (hidden === false) {
        if (this.dragging === false || (this.dragging === true && this.options.hideEdgesOnDrag === false)) {
          this._drawEdges(ctx, inactiveEdges);
        }
      }

      if (this.dragging === false || (this.dragging === true && this.options.hideNodesOnDrag === false)) {
        this._drawNodes(ctx, inactiveNodes, hidden);
      }

      if (hidden === false) {
        if (this.dragging === false || (this.dragging === true && this.options.hideEdgesOnDrag === false)) {
          this._drawEdges(ctx, activeEdges);
        }
      }

      if (this.dragging === false || (this.dragging === true && this.options.hideNodesOnDrag === false)) {
        this._drawNodes(ctx, activeNodes, hidden);
      }
